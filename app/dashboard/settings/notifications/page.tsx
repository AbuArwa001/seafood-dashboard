      ...
      <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-4">
            ...
            <Badge
              variant="outline"
              className="text-[10px] font-black uppercase tracking-widest text-amber-600 border-amber-200 bg-amber-50"
            >
              Automated Alerts
            </Badge>
          </div>
        ...
        >
          <RefreshCw
            className={`h-5 w-5 ${paramsLoading ? "animate-spin" : ""}`}
          />
        </Button>
      </header>
                      ...
                      <div className="flex-1 space-y-6">
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            {param.data_type === "boolean" && (
                              <Badge className="bg-emerald-50 text-emerald-600 border-emerald-100 text-[9px] uppercase tracking-widest px-2 py-0">
                                Global
                              </Badge>
                            )}
                          </div>
                        ...
                        <div className="flex items-center gap-6 p-4 bg-slate-50/50 rounded-2xl border border-slate-100">
                          <div className="bg-white p-2 rounded-lg shadow-sm">
                            {param.key.includes("days") ? (
                              <Clock className="h-4 w-4 text-amber-500" />
                            ) : param.key.includes("email") ? (
                              <Mail className="h-4 w-4 text-blue-500" />
                            ) : param.key.includes("roles") ? (
                          ...
                          <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                            ...
                            <span className="text-slate-600">{param.key}</span>
                          </span>
                        </div>
                      </div>
                          ...
                          {param.data_type === "boolean" ? (
                            <div className="h-16 flex items-center justify-between px-8 bg-slate-50 rounded-[1.25rem] border border-slate-100 shadow-sm">
                              <span
                                className={cn(
                                  ...
                                  (
                                    pendingChanges[param.key] ?? param.value
                                  ).toLowerCase() === "true"
                                    ? "text-emerald-600"
                                    : "text-slate-400",
                                )}
                            ...
                            <div className="space-y-2 max-h-[200px] overflow-y-auto p-2 bg-slate-50 rounded-[1.25rem] border border-slate-100 no-scrollbar">
                              {roles?.map((role) => {
                                ...
                                try {
                                  isSelected = JSON.parse(currentVal).includes(
                                    role.role_name,
                                  );
                                } catch (e) {}
                                ...
                                return (
                                  <button
                                    ...
                                    onClick={() =>
                                      handleValueChange(
                                        param.key,
                                        toggleRole(currentVal, role.role_name),
                                      )
                                    }
                                    className={cn(
                                      "w-full flex items-center justify-between px-4 py-3 rounded-lg transition-all font-bold text-xs uppercase tracking-tight",
                                      isSelected
                          ...
                          {pendingChanges[param.key] !== undefined && (
                            ...
                            >
                              ...
                              >
                                ...
                                updateMutation.variables?.key === param.key ? (
