    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-slate-500 font-semibold max-w-md text-center">
          Only system administrators have permission to manage roles and
          security protocols.
        </p>
        <Button
          variant="outline"
        ...
        {isLoading ? (
          ...
          <div className="col-span-full py-20 text-center space-y-6">
            ...
            <p className="text-slate-500 max-w-sm mx-auto">
              There was an error communicating with the authentication server.
              Please try again.
            </p>
          </div>
        ) : (
                ...
                <CardContent className="p-8 pt-4 flex-1">
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2 max-h-[160px] overflow-y-auto pr-2 custom-scrollbar">
                      {role.permissions.map((perm) => {
                        const { action, module, color } = formatPermission(
                          perm.codename,
                        );
                        return (
                          <div
                            key={perm.id}
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[10px] font-black uppercase tracking-wider ${color}`}
                          >
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-50 flex items-center justify-between text-slate-400">
                    <span className="text-[10px] font-black uppercase tracking-widest">
                      Modified:{" "}
                      {role.updated_at
                        ? format(new Date(role.updated_at), "MMM dd, yyyy")
                        : "Recently"}
                    </span>
