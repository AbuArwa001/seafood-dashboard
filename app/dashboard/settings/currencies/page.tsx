...
export default function CurrencyRatesSettings() {
  ...
  const deleteCurrency = useMutation({
    mutationFn: (id: string) =>
      apiClient.delete(`${API_ENDPOINTS.CURRENCIES}${id}/`),
    onSuccess: () => {
  ...
  const deleteMargin = useMutation({
    mutationFn: (id: string) =>
      apiClient.delete(`${API_ENDPOINTS.MARGINS}${id}/`),
    onSuccess: () => {
    ...
    >
      ...
      >
        <TabsList className="bg-slate-100/50 p-1.5 rounded-[1.5rem] h-16 border border-slate-200/60 w-full sm:w-auto">
          <TabsTrigger
            value="currencies"
            className="rounded-lg px-8 font-black text-sm data-[state=active]:bg-white data-[state=active]:shadow-lg active:scale-95 transition-all"
          >
            CURRENCIES
          </TabsTrigger>
          <TabsTrigger
                  ...
                  <Table>
                    <TableHeader className="bg-slate-50/50">
                      <TableRow className="hover:bg-transparent border-slate-100">
                        <TableHead className="px-8 font-black text-slate-900 uppercase tracking-tighter text-xs">
                          ISO Code
                        </TableHead>
                        <TableHead className="font-black text-slate-900 uppercase tracking-tighter text-xs">
                          Currency Name
                        </TableHead>
                        <TableHead className="text-center font-black text-slate-900 uppercase tracking-tighter text-xs">
                          Symbol
                        </TableHead>
                        <TableHead className="text-center font-black text-slate-900 uppercase tracking-tighter text-xs">
                          Status
                        </TableHead>
                        <TableHead className="text-right px-8 font-black text-slate-900 uppercase tracking-tighter text-xs">
                          Actions
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                          ...
                          >
                            ...
                            <TableCell className="text-center">
                              <div className="flex items-center justify-center">
                                <div
                                  className={`h-2 w-2 rounded-full mr-2 ${curr.is_active ? "bg-emerald-500 animate-pulse" : "bg-slate-300"}`}
                                />
                                <span
                                  className={`text-[10px] font-black uppercase tracking-widest ${curr.is_active ? "text-emerald-500" : "text-slate-400"}`}
                                >
                ...
                <CardContent className="p-0">
                  <Table>
                    <TableHeader className="bg-slate-100/30">
                      <TableRow className="hover:bg-transparent border-slate-100">
                        <TableHead className="px-8 font-black text-slate-900 text-xs">
                          PAIR
                        </TableHead>
                        <TableHead className="font-black text-slate-900 text-xs text-center">
                          VALUE
                        </TableHead>
                        <TableHead className="font-black text-slate-900 text-xs">
                          EFFECTIVE DATE
                        </TableHead>
                        <TableHead className="font-black text-slate-900 text-xs text-right px-8">
                          STABILITY
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                          ...
                          >
                            <TableCell className="px-8 py-6">
                              <div className="flex items-center space-x-3">
                                <div className="bg-blue-600 text-white font-black px-3 py-1 rounded-lg text-[10px] shadow-sm">
                                  {rate.from_currency.code}
                                </div>
                                <ChevronRight className="h-3 w-3 text-slate-300" />
                                <div className="bg-slate-200 text-slate-700 font-black px-3 py-1 rounded-lg text-[10px] shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {loadingMargins ? (
                  ...
                  margins?.map((margin: any) => (
                    <Card
                      key={margin.id}
                      className="border-none shadow-[0_20px_50px_-15px_rgba(0,0,0,0.05)] bg-white rounded-[1.5rem] overflow-hidden border border-slate-50 hover:shadow-xl transition-all duration-300 group ring-1 ring-slate-100/50"
                    >
                      <CardContent className="p-8">
                        ...
                        <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                          <div className="flex items-center">
                            <div
                              className={`h-2.5 w-2.5 rounded-full mr-3 ${margin.is_active ? "bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.5)] animate-pulse" : "bg-slate-300"}`}
                            />
                            <span className="text-xs font-black text-slate-900 uppercase tracking-tight">
                              {margin.is_active ? "Operational" : "Paused"}
                            </span>
                          </div>
                          <div className="flex items-center space-x-2">
                            ...
                            <button
                              onClick={() =>
                                confirm("Remove margin rule?") &&
                                deleteMargin.mutate(margin.id)
                              }
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h3 className="text-3xl font-black mb-6 leading-tight">
                Treasury{" "}
                <span className="text-blue-400 italic">Protection</span>{" "}
                Protocol
              </h3>
              <p className="text-slate-400 font-semibold text-lg max-w-lg mb-8">
                All conversions in the SeaFood Registry apply a weighted margin
                to mitigate local currency volatility risk.
                <span className="text-white">
                  {" "}
                  Admin overrides are logged and audited automatically.
                </span>
              </p>
              <div className="flex flex-wrap gap-4">
                ...
                <div className="bg-white/5 border border-white/10 px-6 py-4 rounded-2xl">
                  ...
                  <p className="font-black text-xl text-emerald-400 underline decoration-emerald-400/30 underline-offset-4">
                    ENABLED
                  </p>
                </div>
              </div>
