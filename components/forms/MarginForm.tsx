...
export function MarginForm({ onSuccess, initialData }: MarginFormProps) {
  ...
  const form = useForm<z.infer<typeof marginSchema>>({
    ...
    defaultValues: {
      from_currency:
        typeof initialData?.from_currency === "object"
          ? initialData.from_currency.id
          : initialData?.from_currency || "",
      to_currency:
        typeof initialData?.to_currency === "object"
          ? initialData.to_currency.id
          : initialData?.to_currency || "",
      margin_percentage: initialData
        ? parseFloat(initialData.margin_percentage)
        : 2.0,
  ...
  const mutation = useMutation({
    mutationFn: async (values: z.infer<typeof marginSchema>) => {
      if (initialData?.id) {
        const response = await apiClient.put(
          `${API_ENDPOINTS.MARGINS}${initialData.id}/`,
          values,
        );
    ...
    onSuccess: () => {
      toast.success(
        initialData ? "Margin updated" : "Margin configured successfully",
      );
    ...
    onError: (error: any) => {
      const errorMessage =
        error.response?.data?.detail || "Failed to save margin configuration";
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem
              className="flex flex-row items-center space-x-3 space-y-0 p-4 bg-slate-50 rounded-2xl cursor-pointer"
              onClick={() => field.onChange(!field.value)}
            >
              <FormControl>
                <div
                  className={`h-5 w-5 rounded flex items-center justify-center border-2 transition-colors ${field.value ? "bg-primary border-primary" : "bg-white border-slate-300"}`}
                >
                  {field.value && (
              ...
              <div className="space-y-1 leading-none select-none">
                <FormLabel className="font-bold text-slate-700 cursor-pointer">
                  Active
                </FormLabel>
